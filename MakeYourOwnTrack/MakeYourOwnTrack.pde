...
void draw() {
  ...
  else
  {
    ...
    else
    {
      ...
      float total_score = constrain(map( score, -max_score, max_score, 0, 100 ), 0, 100);
      if(hardMode == 1)
      {
        final_score = nf(total_score, 2, 1) + "%";
      }
      else
      {  
        final_score =  "100%";
      }
      fill(buttonColors[ round( 0.05 * total_score )]);
      completed = true;
    }
    }
  }
